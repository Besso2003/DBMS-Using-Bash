...
while true; do
    ...
    if [ -z "$table_name" ]; then
        left_text "${RED}Error: Table name cannot be empty.${RESET}"
    elif [ ! -f "$meta_file" ] || [ ! -f "$data_file" ]; then
        left_text "${RED}Error: Table '$table_name' does not exist.${RESET}"
    else
        break
    fi
done
...
while true; do
    ...
    if [[ "$choice" -eq 1 || "$choice" -eq 2 ]]; then break; fi
